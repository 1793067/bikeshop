...
psql -U postgres -tc "SELECT 1 FROM pg_database WHERE datname = 'bikeshop'" | grep -q 1 || psql -U postgres -c "CREATE DATABASE bikeshop"


PSQL="psql -X --username=postgres --dbname=bikeshop --tuples-only -c"

# path to sql tables-builder file
PATH_TO_SQL=$(realpath db.sql | sed 's/\/d\//D:\//')

# create tables to bikeshop db (in not exist)
CREATE_TABLE_BIKES=$($PSQL "\i '$PATH_TO_SQL'")
